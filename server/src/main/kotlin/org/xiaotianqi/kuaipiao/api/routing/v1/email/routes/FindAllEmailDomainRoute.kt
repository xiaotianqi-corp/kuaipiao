...
fun Route.findAllEmailDomainRoute() {
    val resendClient by inject<ResendClient>()

    authenticate(AuthenticationMethods.BEARER_AUTH) {

        get<FindAllEmailDomainRoute> {
            try {
                val domains = resendClient.listDomains()

                if (domains != null) {
                    call.respond(HttpStatusCode.OK, mapOf("data" to domains))
                } else {
                    call.respond(HttpStatusCode.InternalServerError, mapOf("error" to "Failed to list domains"))
                }
            } catch (e: Exception) {
